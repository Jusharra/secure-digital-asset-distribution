...
function AssetDetailsModal({ asset, onClose, onUpdate, userRole, userId }: AssetDetailsModalProps) {
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      // First update the asset's published status
      ...
      // If we're publishing (not unpublishing), create prepaid cards
      if (!asset.published) {
        // Create initial batch of prepaid cards (e.g., 10 cards)
        const initialBatch = Array(10).fill({
          asset_id: asset.id,
          serial_code: crypto.randomUUID(),
          redeemed: false,
      ...
      toast.success(asset.published ? 'Asset unpublished' : 'Asset published with initial cards');
      onUpdate();
    } catch (err) {
              ...
              <div className="flex items-center text-sm">
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      onClick={() => {
                        if (asset.encryption_keys?.[0]?.public_key) {
                          navigator.clipboard.writeText(asset.encryption_keys[0].public_key);
                  ...
                  <code className="text-xs bg-gray-100 px-2 py-1 rounded mt-1 block overflow-x-auto">
                    {asset.encryption_keys?.[0]?.public_key || 'No key available'}
                  </code>
                </div>
              </div>
              <div className="flex items-center text-sm">
...
export default function CreatorAssetList() {
